...
getgenv().GG = {
    -- Système de langues multi-langues
    Languages = {
        English = {
        ...
        Spanish = {
            CheckboxEnabled = "Activado",
            CheckboxDisabled = "Desactivado",
            SliderValue = "Valor",
            DropdownSelect = "Seleccionar",
            DropdownNone = "Ninguno",
            DropdownSelected = "Seleccionado",
            ButtonClick = "Clic",
            TextboxEnter = "Entrar",
            ModuleEnabled = "Activado",
            ModuleDisabled = "Desactivado",
            TabGeneral = "General",
            TabSettings = "Ajustes",
            Loading = "Cargando...",
            Error = "Error",
    ...
    Themes = {
        Dark = {
            ...
            AccentColor = Color3.fromRGB(152, 181, 255),
            BorderColor = Color3.fromRGB(52, 66, 89),
            SecondaryColor = Color3.fromRGB(22, 28, 38),
            HighlightColor = Color3.fromRGB(32, 38, 51)
        },
        Light = {
    ...
    Config = {
        ...
        SaveConfig = function(self, fileName, data)
            local success, result = pcall(function()
                ...
                if GG.Config.Settings.Encryption then
                    jsonData = GG.Utils.Security.Encrypt(jsonData, "library_key")
                end
                
                writefile(fileName, jsonData)
                
                -- Gestion des backups
                if self.Settings.MaxBackups > 0 then
                    self:CreateBackup(fileName)
                end
                
                GG.Logger:Debug("Configuration saved:", fileName)
            end)
            
            if not success then
                GG.Logger:Error("Failed to save config:", result)
            end
            return success, result
        end,
        
        LoadConfig = function(self, fileName)
            local success, result = pcall(function()
                if not isfile(fileName) then
                    GG.Logger:Warn("Config file not found:", fileName)
                    return nil
                end
                ...
                if GG.Config.Settings.Encryption then
                    content = GG.Utils.Security.Decrypt(content, "library_key")
                end
                ...
                if data._metadata then
                    GG.Logger:Info("Config loaded:", data._metadata.Version or "Unknown")
                end
                
                return data._data or data
            end)
            
            if not success then
                GG.Logger:Error("Failed to load config:", result)
            end
            return success, result
        end,
        
        CreateBackup = function(self, fileName)
            if isfile(fileName) then
                local timestamp = os.date("%Y%m%d_%H%M%S")
                local backupName = "backups/" .. fileName:gsub(".json", "") .. "_" .. timestamp .. ".json"
                
                if not isfolder("backups") then
                    makefolder("backups")
                end
                
                writefile(backupName, readfile(fileName))
                GG.Logger:Debug("Backup created:", backupName)
            end
        end,
        
        CleanOldBackups = function(self, fileName)
            local backups = {}
            local pattern = fileName:gsub(".json", "") .. "_(%d+)_(%d+)%.json"
            
            if isfolder("backups") then
                for _, file in ipairs(listfiles("backups")) do
                ...
                while #backups > self.Settings.MaxBackups do
                    local oldBackup = table.remove(backups, 1)
                    delfile(oldBackup)
                    GG.Logger:Debug("Old backup removed:", oldBackup)
                end
            end
        end
    },
    ...
    Logger = {
        ...
        CurrentLevel = 2,
        LogToFile = false,
        LogFile = "nury_logs.txt",
        
        Log = function(self, level, message, ...)
            if self.LogLevels[level] >= self.CurrentLevel then
                ...
                if select("#", ...) > 0 then
                    ...
                    for i, v in ipairs(args) do
                        if type(v) == "table" then
                            args[i] = HttpService:JSONEncode(v)
                        end
                    end
                    formatted = formatted .. " " .. table.concat(args, " ")
                end
                
                print(formatted)
                
                if self.LogToFile then
                    self:WriteToLogFile(formatted)
                end
        ...
        WriteToLogFile = function(self, message)
            local success, result = pcall(function()
                if not isfile(self.LogFile) then
                    writefile(self.LogFile, message .. "\n")
                else
                    appendfile(self.LogFile, message .. "\n")
                end
            end)
            
            if not success then
                print("Failed to write to log file:", result)
            end
        end,
        
        SetLogLevel = function(self, level)
            if type(level) == "string" then
                self.CurrentLevel = self.LogLevels[level] or 2
            elseif type(level) == "number" then
                self.CurrentLevel = level
            end
            GG.Logger:Info("Log level set to:", self.CurrentLevel)
        end,
        
        ClearLogFile = function(self)
            if isfile(self.LogFile) then
                writefile(self.LogFile, "")
                GG.Logger:Info("Log file cleared")
            end
        end
    },
    ...
    Performance = {
        ...
        StartMonitoring = function(self)
            ...
            self.MonitorConnection = RunService.Heartbeat:Connect(function(deltaTime)
                -- FPS
                self.Metrics.FPS = math.floor(1/RunService.RenderStepped:Wait())
                
                -- Utilisation mémoire
                self.Metrics.MemoryUsage = Stats:GetMemoryUsageMb()
                
                -- Latence réseau
                self.Metrics.NetworkLatency = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue()
            ...
            GG.Logger:Info("Performance monitoring started")
        end,
        ...
        StopMonitoring = function(self)
            ...
            GG.Logger:Info("Performance monitoring stopped")
        end,
        
        Optimize = function(self)
            GG.Logger:Info("Starting performance optimization")
            ...
            -- Désactiver les détails inutiles
            workspace.Terrain:SetMaterialColor(Enum.Material.Grass, Color3.new(0,0,0))
            
            GG.Logger:Info("Performance optimization completed")
        end,
        ...
        GetReport = function(self)
            return string.format(
                ...
                self.Metrics.NetworkLatency,
                self.Metrics.RenderTime
            )
        end,
        
        IsPerformanceGood = function(self)
            return self.Metrics.FPS >= 30 and 
                   self.Metrics.MemoryUsage < 500 and 
                   self.Metrics.NetworkLatency < 200
        end
    },
    ...
    Plugins = {
        ...
        RegisterPlugin = function(self, pluginName, pluginData)
            self.Available[pluginName] = pluginData
            GG.Logger:Info("Plugin registered:", pluginName)
        end,
        
        LoadPlugin = function(self, pluginName)
            if self.Loaded[pluginName] then
                GG.Logger:Warn("Plugin already loaded:", pluginName)
                return false
            end
            
            local plugin = self.Available[pluginName]
            if not plugin then
                GG.Logger:Error("Plugin not found:", pluginName)
                return false
            end
            ...
            if success then
                self.Loaded[pluginName] = instance
                GG.Logger:Info("Plugin loaded successfully:", pluginName)
                return true
            else
                GG.Logger:Error("Failed to load plugin:", pluginName, instance)
                return false
            end
        end,
        
        UnloadPlugin = function(self, pluginName)
            if not self.Loaded[pluginName] then
                GG.Logger:Warn("Plugin not loaded:", pluginName)
                return false
            end
            
            local plugin = self.Loaded[pluginName]
            if plugin.OnUnload and type(plugin.OnUnload) == "function" then
                local success, result = pcall(plugin.OnUnload, plugin)
                if not success then
                    GG.Logger:Error("Error in plugin OnUnload:", result)
                end
            end
            
            self.Loaded[pluginName] = nil
            GG.Logger:Info("Plugin unloaded:", pluginName)
            return true
        end,
    ...
    Utils = {
        ...
        Table = {
            DeepCopy = function(t)
                ...
                for k, v in pairs(t) do
                    copy[k] = GG.Utils.Table.DeepCopy(v)
                end
                return copy
            end,
            
            Merge = function(t1, t2)
                local result = GG.Utils.Table.DeepCopy(t1)
                for k, v in pairs(t2) do
                    if type(v) == "table" and type(result[k]) == "table" then
                        result[k] = GG.Utils.Table.Merge(result[k], v)
                    else
                        result[k] = v
                    end
                end
        ...
        Security = {
            ...
            Encrypt = function(text, key)
                -- Chiffrement simple (XOR) - À améliorer pour un usage sérieux
                local result = ""
                local keyLen = #key
                for i = 1, #text do
                    local keyChar = string.byte(key, (i - 1) % keyLen + 1)
                    local textChar = string.byte(text, i)
                    result = result .. string.char(bit32.bxor(textChar, keyChar))
                end
                return result
            end,
            
            Decrypt = function(text, key)
                return GG.Utils.Security.Encrypt(text, key) -- XOR est symétrique
            end
        },
        ...
        UI = {
            CreateTooltip = function(text, parent)
            ...
            CreateNotification = function(title, message, duration, color)
                GG.Logger:Info("Notification:", title, message)
            ...
            CreateLoadingSpinner = function(parent)
                ...
                circle.Image = "rbxassetid://11983271915"
        ...
        FileSystem = {
            EnsureFolder = function(folderPath)
                local folders = GG.Utils.String.Split(folderPath, "/")
                local currentPath = ""
                ...
                for _, folder in ipairs(folders) do
                    ...
                    if not isfolder(currentPath) then
                        ...
                        GG.Logger:Debug("Created folder:", currentPath)
                    end
                end
            end,
            
            SafeWrite = function(filePath, content)
                ...
                GG.Logger:Debug("File written safely:", filePath)
            end,
            ...
            ReadJSON = function(filePath)
                ...
                if not success then
                    GG.Logger:Error("Failed to read file:", filePath)
                    return nil
                end
                
                local success2, data = pcall(HttpService.JSONDecode, HttpService, content)
                if not success2 then
                    GG.Logger:Error("Failed to parse JSON:", filePath)
                    return nil
                end
            ...
            WriteJSON = function(filePath, data)
                ...
                if not success then
                    GG.Logger:Error("Failed to encode JSON:", filePath)
                    return false
                end
                
                GG.Utils.FileSystem.SafeWrite(filePath, json)
                return true
            end
        }
    },
    ...
    Initialize = function(self)
        GG.Logger:Info("Initializing GG Library v2.0.0")
        ...
        GG.Utils.FileSystem.EnsureFolder("Nury")
        GG.Utils.FileSystem.EnsureFolder("Nury/backups")
        GG.Utils.FileSystem.EnsureFolder("Nury/plugins")
        
        GG.Logger:Info("GG Library initialized successfully")
    ...
    Shutdown = function(self)
        GG.Logger:Info("Shutting down GG Library")
        
        -- Arrêter le monitoring
        self.Performance:StopMonitoring()
        
        -- Décharger tous les plugins
        for pluginName in pairs(self.Plugins.Loaded) do
            self.Plugins:UnloadPlugin(pluginName)
        end
        
        -- Sauvegarder la configuration
        if self.Config.Settings.AutoSave then
            -- Sauvegarder la configuration globale
            local configData = {
                language = self.CurrentLanguage,
                theme = self.CurrentTheme,
                logLevel = self.Logger.CurrentLevel
            }
            
            self.Config:SaveConfig("Nury/gg_config.json", configData)
        end
        
        GG.Logger:Info("GG Library shutdown complete")
    end
}
...
-- Définir la langue courante pour la bibliothèque
GG.Language = GG.Languages[GG.CurrentLanguage]

-- Fonction utilitaire pour convertir les chaînes en tables
function convertStringToTable(inputString)
    GG.Logger:Debug("Converting string to table:", inputString)
    local result = {}
...
-- Fonction utilitaire pour convertir les tables en chaînes
function convertTableToString(inputTable)
    GG.Logger:Debug("Converting table to string:", #inputTable, "items")
    
    if not inputTable or #inputTable == 0 then
        return ""
    end
...
-- Services globaux avec cloneref pour la sécurité
GG.Logger:Debug("Utilities initialized")
    ...
    callback = function(value)
        if value then
            ...
            heartbeatConn = RunService.Heartbeat:Connect(function()
                ...
                -- Find the fastest ball
                for _, Ball in ipairs(Balls) do
                    local zoomies = Ball:FindFirstChild("zoomies")
                    if zoomies then
                        local speed = math.floor(zoomies.VectorVelocity.Magnitude)
                        if speed > currentVelocity then
                ...
                if currentBall then
                    -- Update peak
                    ballPeaks[currentBall] = ballPeaks[currentBall] or 0
                    if currentVelocity > ballPeaks[currentBall] then
                        ballPeaks[currentBall] = currentVelocity
                    end
                ...
                -- Clean up old balls
                for oldBall in pairs(ballPeaks) do
                    if not oldBall or not oldBall.Parent then
                        ballPeaks[oldBall] = nil
                    end
                end
            end)
            
            ballStatsUI.Enabled = true
        else
