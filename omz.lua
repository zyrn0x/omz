...
-- Ajoutez ces variables après les déclarations initiales du script Omz
local AntiBanEnabled = false
local AntiBanModule = nil
...
local function InitializeAntiBan()
    if AntiBanEnabled then
        ...
        -- Masquer les éléments GUI
        local function obfuscateGUI()
            local guiElements = game:GetService("CoreGui"):GetChildren()
            for _, gui in ipairs(guiElements) do
                if gui.Name == "Omz" or gui.Name:find("Sigma") then
                    pcall(function()
        ...
        -- Vérifier les joueurs existants
        for _, player in pairs(game.Players:GetPlayers()) do
            if player ~= game.Players.LocalPlayer then
                local isStaff, reason = isWiggityStaff(player)
                if isStaff then
                    warn("⚠️ Staff détecté: " .. player.Name .. " (" .. reason .. ")")
                    
                    -- Désactiver les features
                    if AutoParryModule and AutoParryModule._state then
                        AutoParryModule:change_state(false)
                    end
                    
                    if AutoSpamModule and AutoSpamModule._state then
                        AutoSpamModule:change_state(false)
                    end
                    
                    -- Kick
                    game.Players.LocalPlayer:Kick("Staff détecté - Sécurité")
                    return
                end
            end
        end
        
        -- Surveiller nouveaux joueurs
        AntiBanConnections.playerAdded = game.Players.PlayerAdded:Connect(function(player)
            task.wait(1) -- Attendre un peu
            local isStaff, reason = isWiggityStaff(player)
            if isStaff then
                warn("⚠️ Nouveau staff détecté: " .. player.Name .. " (" .. reason .. ")")
                ...
                game.Players.LocalPlayer:Kick("Nouveau staff détecté - Sécurité")
            end
        end)
        
        -- Surveiller commandes de chat
        AntiBanConnections.chat = game.Players.LocalPlayer.Chatted:Connect(function(message)
            local msg = message:lower()
            if msg:find("!ban") or msg:find("!kick") or msg:find("!admin") or msg:find("!mod") then
                warn("⚠️ Commande staff détectée: " .. message)
                ...
                game.Players.LocalPlayer:Kick("Commande staff détectée - Sécurité")
            end
        end)
        ...
        AntiBanConnections.network = game:GetService("RunService").Heartbeat:Connect(function()
            local now = tick()
            if now - lastNetworkCheck >= 1 then
                networkSent = 0
        ...
        print("🔓 Anti Ban désactivé")
        -- Nettoyer
        for name, conn in pairs(AntiBanConnections) do
            if conn then
...
-- Maintenant, créez le module dans l'interface Omz
-- Trouvez où vous créez les tabs (probablement vers la fin du script)

-- Exemple d'intégration dans la tab "misc" ou "visuals":
local antibanModule = misc:create_module({
    title = 'Anti Ban',
    flag = 'AntiBan',
    description = 'Protection contre la détection staff',
    section = 'left', -- ou 'right' selon votre layout
...
-- Stocker la référence pour pouvoir la désactiver plus tard
AntiBanModule = antibanModule

-- Optionnel: Ajouter une checkbox pour les notifications
antibanModule:create_checkbox({
    title = "Notifications",
    flag = "AntiBanNotify",
    callback = function(value)
        getgenv().AntiBanNotify = value
    end
})

-- Optionnel: Ajouter une checkbox pour obfuscation GUI
antibanModule:create_checkbox({
    title = "Obfuscation GUI",
    flag = "AntiBanObfuscate",
    callback = function(value)
        getgenv().AntiBanObfuscate = value
    end
})
